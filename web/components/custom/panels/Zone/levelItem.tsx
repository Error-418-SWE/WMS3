...
export default function LevelItem({
	...
	return (
		<div
			className={
				"border p-3 rounded-md bg-slate-100 group hover:bg-slate-200 focus-within:bg-slate-200"
			}
		>
				...
				{removable ? (
					<Button
						...
						className={
							...
							" text-black hover:bg-destructive focus:bg-destructive hover:text-white focus:text-white"
						}
						onClick={() => {
								...
								<Input
									...
									onChange={(e) => {
										...
										const newLevels = levels.map((l) =>
											l.id === level.id
												? { ...l, height: parseFloat(e.target.value) }
												: l,
										);
										setLevels(newLevels);
									}}
